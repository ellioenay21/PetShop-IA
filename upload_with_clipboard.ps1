# Upload do projeto para GitHub usando PAT do clipboard
param(
...
Write-Host "Lendo PAT da área de transferência..."
try { $pat = Get-Clipboard -TextFormatType Text } catch { $pat = $null }
if (-not $pat) { Write-Error "Área de transferência vazia. Copie o PAT e execute novamente."; exit 1 }
...
try {
  ...
  if ($_.Exception -and $_.Exception.Response -and $_.Exception.Response.StatusCode.Value__ -eq 422) {
    Write-Host "Repositório já existe. Prosseguindo..."
  } else {
    Write-Error "Falha ao criar repositório: $($_.Exception.Message)"; exit 1
  }
}
...
foreach ($f in $files) {
  ...
  $putBody = @{ message = "Add $relPath"; content = $b64; branch = "main" } | ConvertTo-Json
  ...
  try {
    ...
    try {
      ...
      $putBody2 = @{ message = "Update $relPath"; content = $b64; sha = $existing.sha; branch = "main" } | ConvertTo-Json
